...
## Optional uncertainty-v1 extension

Schema 3 also accepts a separate export generated by
`python -m coolchange.gwr_uncertainty` in the model repository. Original exports
remain supported. No database migration is needed: the extension is stored in
existing JSONB metadata/payloads. Importing a new file activates its new SHA-256
release, so deploy the updated backend before importing the extended export.
A metadata flag alone does not add intervals to an old release.

| Field | Meaning |
| --- | --- |
| `simulator.uncertainty` | Method, nominal level, assumptions, source hashes and fit diagnostics; present in bootstrap and block responses only for extended releases |
| `tree_planting.coefficient_uncertainty` | `standard_error`, `lower`, `upper` for the block's slope in °C per canopy percentage point; `null` for unavailable blocks |
| `scenarios[i].cooling_interval` | `lower_c`, `upper_c`, `includes_zero`, `level: 0.95`, `method: gwr_coefficient_normal_v1`, `coverage_validated: false` |

Baseline, no-change and unavailable scenarios have `cooling_interval: null`.
Old releases omit the extension entirely. Absence/null means no interval to show;
it is not a zero-width uncertainty band. Preserve `policy.validated_uncertainty_interval: false`.

For whole-tree requests, first apply all existing count, support and screening
rules. For an eligible positive canopy increase `delta_pp`, use the selected
block's `coefficient_uncertainty`:

```js
const lower_c = -coefficient.upper * delta_pp;
const upper_c = -coefficient.lower * delta_pp;
const includes_zero = lower_c <= 0 && upper_c >= 0;
```

`delta_pp` is in percentage points (5, not 0.05). Do not use baseline UHI RMSE as
the standard error, add baseline prediction noise, clip a negative bound to zero,
or label a zero-crossing interval as clear cooling. The coefficient endpoints
reverse order because cooling is the negative of slope times canopy addition.
The Python `simulate_trees --export ...` command provides the reference result.

Suggested display: **Approximate 95% model-based interval (coverage not validated)**.
Show the metadata notice with the estimate. These are pointwise intervals for a
conditional mean association, given fixed bandwidth, canopy change and observed
baseline. They exclude spatial error correlation, smoothing and screening bias,
bandwidth selection, measurement error, growth/survival, site capacity and future
observation noise. They are neither simultaneous intervals across all blocks nor
causal or actual planting-outcome guarantees. Residual spatial correlation was
found in the current data, so these bands may be too narrow.

The importer rejects inconsistent level/method claims, negative standard errors,
incorrect coefficient or cooling bounds, hidden negative lower bounds and intervals
on unavailable/baseline scenarios. This validates structure/arithmetic, not
statistical coverage. See [uncertainty validation](UNCERTAINTY_VALIDATION.md).

For an isolated local copy, the read-only verifier additionally accepts the exact
database name `coolchange_uncertainty_test` on localhost/127.0.0.1 port 5433.
It does not create, seed, import or change a database itself.
