...
An explicit HTTPS URL or local JSON path may be passed after `--` for a controlled import. `--dry-run` downloads and validates without accessing the database. Downloads require HTTPS, have a two-minute timeout and a 150 MB limit; failed downloads leave the active database release untouched. Re-run the import when the S3 object is updated.
